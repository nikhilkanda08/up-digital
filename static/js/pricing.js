...
//var values = [50, 100, 150, 200, 300, 500, 1000, 1500, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 10000, 20000, 30000, 40000, 50000];
$(document).ready(function(){
    console.log("Load pricing javascript");
    var current_plan = $('#current_plan').val();
    var new_values = values.filter(function(item) {
    return item > current_plan;
    });
    values = new_values;
    ...
    $("#handle1").roundSlider({
        ...
        min: 0,
//        max: 21,
        max: parseInt(new_values.length)-1,
        // value: 25,
        // min: 25,
        // max: 50000,
        handleSize: 0,
    ...
    /* added new */
         let slider_new = values[0];
          let planIndexObj_new = {}
          $.getJSON( "/static/json/searchPlan.json", function( data ) {
            $.each( data, function( key, val ) {
              $.each(val, function( index, value ) {
                if(slider_new >= value['dailySearches']){
                  planIndexObj_new = value;
                }
              });
            });
          });
             setTimeout(() => {
           $("#monthly_cost").text(planIndexObj_new['cost']);
    }, 1000);
...
function checkout_paypal(){
  let palnID = $("#price_plan").val();
  console.log(palnID);
  window.location.href = window.location.protocol + "//" + window.location.host + "/process-payment/"+palnID+"/";
}
